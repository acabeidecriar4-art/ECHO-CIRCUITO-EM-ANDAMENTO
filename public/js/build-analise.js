...
import { esc, mStyle, mInner, corDoItem, normalizar } from './ui-core.js?pgv=82901afbc8df';
import {
  applyEquipmentStats,
  buildRealStatLines,
  calculateShotDistribution
} from './game-stat-engine.js?v=14&pgv=82901afbc8df';
import { evaluateAppliedModifiersForHeroV4 } from './echo-brain-item-fit-v4.js?v=4&pgv=82901afbc8df';
import {
  getEquipmentFreshness,
  equipmentFreshnessState
} from './echo-brain-equipment-freshness.js?v=1&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
import { scorePublicItemFitV4 } from './echo-brain-item-fit-client-v4.js?v=20260824-item-fit-authority-1&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
...
export async function carregarDadosAnalise() {
  ...
  try {
    const { data, error } = await supabase.from('hero_skills').select(
      'hero_id,name,description,skill_type,cooldown,duration,enabled,verification_status,needs_recheck'
    );
    if (error) throw error;
...
function avaliarEchoBrain(contexto, base, calculo) {
  ...
  for (const [slot, item] of Object.entries(equipados || {})) {
    ...
    const itemCalc = hasStats
      ? applyEquipmentStats(base, [stats])

...
export function expiryLabel(value) {
  if (!value) return 'Validade não informada pela fonte';
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return 'Confira a validade na publicação oficial';
...
export function giftMarkupFor(promo) {
  ...
  const sourceName = escapeHtml(promo.source_name || 'Publicação oficial');
  return `<dialog class="echo-promo-dialog" aria-labelledby="echo-promo-title" aria-describedby="echo-promo-description">
    <article class="echo-promo-sheet">
      ...
      <div class="echo-promo-reward" hidden><span class="echo-promo-unlocked">${HEART} SEU CÓDIGO PROMOCIONAL</span><div class="echo-promo-code-box" tabindex="0" aria-label="Código promocional revelado"></div><button class="echo-promo-copy-button" type="button">Copiar código</button><p class="echo-promo-redemption">Copiar não resgata automaticamente. Siga as instruções da publicação oficial para usar o código no jogo.</p></div>
...
export function createPromoGiftView(promo, {onAction, onMinimize = () => {}, win = window, doc = document} = {}) {
  if (!doc.getElementById(STYLE_ID)) {
    ...
    link.id = STYLE_ID;
    link.rel = 'stylesheet';
    link.href = new URL('../css/public-promo-code.css?v=20260828-promo-gift-5', import.meta.url).href;
  ...
  function completeReveal() {
    ...
    find('#echo-promo-description').textContent = 'Seu código está pronto. Copie e confira o resgate na fonte oficial.';
  ...
  function expire() {
    ...
    launcher.hidden = true;
    message('Consulte a publicação oficial para mais informações.');
  }
  ...
  copy.addEventListener('click', async () => {
    ...
    try {
      ...
      message(promo.__preview ? 'Prévia copiada — sem valor de resgate.' : 'Código copiado. Siga as instruções da fonte para resgatar no jogo.');

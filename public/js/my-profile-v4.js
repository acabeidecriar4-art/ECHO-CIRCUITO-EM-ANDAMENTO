...
import { signupHandleFromUser } from './identity-public-launch-v6.js?v=20260825-identity-public-v6-1&pgv=82901afbc8df';
import {
  COMMUNITY_TIER_GLYPHS,
  communityTierState,
  pointPreviewValues,
  tierRequirementParts
} from './my-profile-experience-v7.js?v=20260825-identity-experience-v7-1&pgv=82901afbc8df';
import {
...
} from './my-profile-alive-v8.js?v=20260825-identity-alive-v8-1&pgv=82901afbc8df';
...
function renderPreviewJourney(reputation, badges, availability = {}) {
  ...
  $('preview-tier').textContent = availability.reputationUnavailable
    ? 'INDISPONÍVEL'
    : knownTier ? TIER_LABELS[tier].toUpperCase() : 'SEM NÍVEL CONFIRMADO';
  syncIdentityAliveJourney({
    tier: knownTier ? tier : '',
    label: availability.reputationUnavailable ? 'INDISPONÍVEL' : knownTier ? TIER_LABELS[tier] : 'SEM NÍVEL CONFIRMADO',
...
function renderJourney(reputation, badges, policyMeta = null, policyRules = [], availability = {}) {
  $('my-scout-tier').textContent = reputation ? (TIER_LABELS[reputation.community_tier] || reputation.community_tier || 'Member') : '—';
  $('my-scout-points').textContent = reputation ? formatNumber(reputation.reputation_points) : '—';
  $('my-scout-acceptance').textContent = reputation ? formatPercent(reputation.acceptance_rate,true,reputation.decided_count) : '—';
  $('my-scout-verified').textContent = reputation ? formatNumber(reputation.verified_count) : '—';
  $('my-scout-corroborated').textContent = reputation ? formatNumber(reputation.corroborated_count) : '—';
  $('my-scout-first').textContent = reputation ? formatNumber(reputation.first_discoveries) : '—';
  renderScoutTrack(reputation?.community_tier || '',policyRules,reputation);
  ...
  renderReputationPolicy(policyMeta,policyRules,reputation);
  renderPreviewJourney(reputation,badges,availability);
...
async function loadJourney() {
  const [repResult,badgesResult,metaResult,rulesResult] = await Promise.all([
    supabase.from('echo_community_reputation').select('community_tier,reputation_points,verified_count,corroborated_count,first_discoveries,acceptance_rate,accepted_count,decided_count,policy_version').eq('user_id',currentUserId).maybeSingle(),

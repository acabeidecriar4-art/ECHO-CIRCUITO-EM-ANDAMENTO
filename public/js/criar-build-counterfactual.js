...
} from './echo-brain-build-counterfactual-v1.js?v=1&pgv=82901afbc8df';
...
function compatible(row, hero) {
  if (!row || !hero?.databaseId) return false;
  if (row.hero_id) return String(row.hero_id) === String(hero.databaseId);
  if (row.class_id) return String(row.class_id) === String(hero.classId || '');
  return true;
}

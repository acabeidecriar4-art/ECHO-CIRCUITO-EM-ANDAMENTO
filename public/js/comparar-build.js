...
import { esc, numSafe, corDoItem, nivelAtual, normalizar } from './ui-core.js?pgv=82901afbc8df';
import { carregarDadosAnalise, analisarBuildComAutoridadeV4 } from './build-analise.js?v=16&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
import { STAT_DEFINITIONS, applyEquipmentStats } from './game-stat-engine.js?v=14&pgv=82901afbc8df';
...
/* ============================================================
   ...
   Fonte dos números: o mesmo build-analise.js usado em Criar Build.
...
const CATEGORY_KEYS = {
  ofensiva: new Set([
    'damage_per_shot', 'armor_penetration', 'penetration_power',
    'health_damage_multiplier', 'armor_drone_multiplier',
    'firepower_summary', 'armor_break_summary', 'fire_rate_summary'
  ]),
  defesa: new Set(['health', 'armor', 'armor_resistance', 'penetration_resistance']),
  mobilidade: new Set(['max_movement_speed', 'aimed_movement_speed']),
  utilidade: new Set([
    'vision_range', 'aimed_range', 'hip_fire_range', 'reload_time',
    'magazine_size', 'ammo_capacity_summary', 'effective_range_summary',
    'aiming_stability_summary'
  ])
};

const STAT_DESCRIPTIONS = {
  damage_per_shot: 'Dano real causado por cada disparo antes das resistências do alvo.',
  health: 'Quantidade total de vida do herói após os modificadores equipados.',
  armor: 'Armadura disponível após os modificadores equipados.',
  max_movement_speed: 'Velocidade máxima de deslocamento do herói.',
  aimed_movement_speed: 'Velocidade mantida enquanto o herói está mirando.',
  vision_range: 'Distância máxima em que alvos podem ser identificados.',
  armor_penetration: 'Percentual de penetração aplicado contra a armadura inimiga.',
  penetration_power: 'Poder de perfuração final da arma.',
  armor_resistance: 'Resistência final da armadura do herói.',
  health_damage_multiplier: 'Multiplicador de dano aplicado diretamente à vida.',
  armor_drone_multiplier: 'Multiplicador de dano aplicado à armadura e drones.',
  aimed_range: 'Alcance efetivo da arma enquanto o herói está mirando.',
  hip_fire_range: 'Alcance efetivo da arma sem usar a mira.',
  reload_time: 'Tempo necessário para concluir uma recarga; aqui, menor é melhor.',
  magazine_size: 'Quantidade de munição disponível antes de recarregar.'
};
...
function formatStat(value, definition = {}) {
  ...
  return `${definition.prefix || ''}${format(value, definition.decimals ?? 0)}${definition.unit || ''}`;
}
...
function normalizeVariantStats(attributes) {
  if (Array.isArray(attributes)) {
    return Object.fromEntries(attributes
      ...
      .map(attribute => [String(attribute.label).trim(), attribute.value ?? '']));
  }
...
async function loadCatalog() {
  const [
    ...
    legacyLevelsResult, variantsResult, raritiesResult, setsResult,
    bonusesResult, tiersResult, analysisData
  ] = await Promise.all([
    ...
    supabase.from('equipments').select(`
      id,name,description,image_path,image_url,rarity,slot_id,set_id,
    ...
    supabase.from('equipment_rarity_levels').select('equipment_id,rarity_slug,rarity_name,rarity_order,rarity_color,stats').order('rarity_order', { ascending: true }),
  ...
  queryError(legacyLevelsResult, 'níveis antigos', false);
  queryError(variantsResult, 'variantes de equipamento', false);
  queryError(raritiesResult, 'raridades', false);
  queryError(setsResult, 'conjuntos', false);
  queryError(bonusesResult, 'bônus de conjunto', false);

  if (legacyLevelsResult.error && variantsResult.error) {
    throw new Error('Nenhuma fonte de atributos dos equipamentos está disponível.');
  }
  ...
  for (const level of legacyLevelsResult.data || []) addLevel(level);
...
function contributorMap(analysis, equipped, catalog) {
  ...
  for (const item of Object.values(equipped).filter(Boolean)) {
    const level = nivelAtual(item);
    if (!level?.stats) continue;
    const result = applyEquipmentStats(base, [level.stats]);
    result.applied.forEach(entry => add(entry.target, item.nome));
  }

  for (const bonus of analysis?.bonusAtivos || []) {
    const bonusStats = catalog.analysisData.statsBonus?.get(bonus.id);
    if (!bonusStats) continue;
    const result = applyEquipmentStats(base, [bonusStats]);
    result.applied.forEach(entry => add(entry.target, `${bonus.set}: ${bonus.titulo}`));
  }
...
async function materializeBuild(build, rows) {
  ...
  for (const row of rows || []) {
    const item = state.catalog.equipmentById.get(row.equipment_id);
    if (!item) {
      console.warn('[comparar-build] Equipamento ausente do catálogo:', row.equipment_id);
      continue;
    }
    const slot = state.catalog.slots[number(row.slot, 1) - 1]
      || state.catalog.slots.find(candidate => candidate.key === item.slot);
    if (!slot) continue;
    const rarity = state.catalog.tierSlugById.get(row.tier_id) || item.raridade;
    equipped[slot.key] = { ...item, raridade: rarity };
  }

  const analysis = await analisarBuildComAutoridadeV4({
    heroi: { databaseId: hero.databaseId, classe: hero.className },
    slots: state.catalog.slots,
    equipados: equipped,
    dados: state.catalog.analysisData
  }, 'comparison');
  ...
  return {
    ...
    gear,
    analysis,
    contributors: contributorMap(analysis, equipped, state.catalog)
  };
}
...
function renderMine() {
  ...
  const changed = Object.keys(build.analysis.total || {}).filter(key => {
    if (!STAT_DEFINITIONS[key]) return false;
    const base = number(build.analysis.base?.[key], 0);
    return Math.abs(number(build.analysis.total[key], 0) - base) > 1e-9;
  }).length;
  ...
  $('my-metric-three').textContent = build.analysis.estado || '—';
...
function comparisonRows() {
  ...
  return [...new Set([...Object.keys(mineTotal), ...Object.keys(otherTotal)])]
    .filter(key => STAT_DEFINITIONS[key])
    .map(key => {
      const definition = STAT_DEFINITIONS[key];
      const mineValue = number(mineTotal[key], 0);
      const otherValue = number(otherTotal[key], 0);
      ...
      const percent = mineValue ? delta / Math.abs(mineValue) * 100 : (otherValue ? 100 : 0);
      return {
        key,
        definition,

import * as core from './build-analise.js?core=16&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
import {
  applyEquipmentStats,
  STAT_DEFINITIONS
...
function buildAuditTrail(contexto, resultado) {
  ...
  for (const [slotKey, item] of Object.entries(equipados)) {
    ...
    if (!level?.stats || !Object.keys(level.stats).length) continue;
...
function targetDefinition(target) {
  return STAT_DEFINITIONS[target] || {
  ...
  activeBonuses.forEach((bonus, index) => {
    ...
    if (bonusModifiers.length) {
      ...
      effects.innerHTML = bonusModifiers.map(modifier => {
        const definition = targetDefinition(modifier.target);

...
(() => {
  ...
  function normalize(raw) {
    ...
    if(Array.isArray(raw.drafts))d.drafts=raw.drafts.slice(0,20).filter(e=>e&&typeof e==='object'&&clean(e.name,60)).map(e=>({id:clean(e.id,40)||uid('draft'),name:clean(e.name,60),category:['Amador','Profissional','Sindicatos','Especial'].includes(e.category)?e.category:'Amador',format:['Mata-mata','Grupos + finais','Liga'].includes(e.format)?e.format:'Mata-mata',capacity:[8,16,24,32,64].includes(Number(e.capacity))?Number(e.capacity):16,date:validDateOnly(e.date)?e.date:'2026-10-24',time:/^\d{2}:\d{2}$/.test(e.time||'')?e.time:'19:00',description:clean(e.description,400),rules:clean(e.rules,1500),createdAt:clean(e.createdAt,40),updatedAt:clean(e.updatedAt,40)}));

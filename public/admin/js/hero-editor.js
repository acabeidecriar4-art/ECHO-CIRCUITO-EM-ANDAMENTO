...
import { resolveMediaUrl, uploadMedia, removeR2Media } from '../../js/media-storage.js?v=20260823-security-supabase-pin-1&pgv=82901afbc8df';
import { initAdminLocalImageImport } from './admin-ai-image-import.js?v=20260828-hero-ocr-3&nocr=20260828-paddle-browser-v3&parser=20260828-hero-ocr-5&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
...
function buildUpdateDiffs(bundle) {
  ...
  const importedMetaValue = (key, formValue, currentValue) =>
    importedMeta[key] === null || importedMeta[key] === undefined ? currentValue : formValue;
  const diffs = [
    ...
    createDiff({ key:'rarity_id', label:'Raridade', group:'Geral', before:hero.rarity_id, after:importedMetaValue('rarity',fields.rarityId?.value || null,hero.rarity_id), apply:{ type:'hero', column:'rarity_id' } }),
    createDiff({ key:'faction', label:'Facção', group:'Geral', before:hero.faction, after:importedMetaValue('faction',fields.faction?.value.trim() || null,hero.faction), apply:{ type:'hero', column:'faction' } }),
...
function renderVersionHistory() {
  ...
  if (!history.length) {
    return '<section class="update-group"><div class="update-group-head">HISTÓRICO <span>Nenhum snapshot anterior</span></div></section>';
  }
  return `<section class="update-group"><div class="update-group-head">HISTÓRICO <span>${history.length} snapshot(s) persistentes</span></div>${history.map(item => `
...
function renderUpdateAssistant() {
  ...
  const identity = `<div class="update-identity-grid">
    <article class="update-identity-card">${bundle.hero.image_path ? `<img src="${escapeHtml(getPublicUrl(bundle.hero.image_path))}" alt="">` : '<div class="update-avatar-placeholder">◇</div>'}<div><small>HERÓI CADASTRADO</small><h3>${escapeHtml(bundle.hero.name)}</h3><p>${escapeHtml(classNameForId(bundle.hero.class_id))} · banco atual</p></div></article>
  ...
  body.innerHTML = identity + (summary.changed ? groupHtml : '<div class="update-empty"><strong>Nenhuma alteração encontrada</strong><br>Os dados importados são idênticos aos já cadastrados. Nada será atualizado.</div>') + renderVersionHistory();
...
async function openUpdateAssistant(existing) {
  const [bundle, historyResult] = await Promise.all([
    loadExistingHeroBundle(existing.id),
    supabase
...
async function applySelectedUpdate(selectAll = false) {
  ...
  try {
    ...
    const mediaChosen = new Set(
      chosen.filter(item => item.apply.type === 'media').map(item => item.apply.media)
    );
    upload = await uploadSelectedMediaSelective(bundle.hero.slug, mediaChosen);
    ...
    const baseStats = {};
    const weaponStats = {};
    let weaponName = bundle.weaponName || integratedStats.weaponName?.value.trim() || null;
    ...
    await saveHeroBundleV2({
      ...
      source: 'hero-ai-selected-update'
    });

...
} from './equipment-audit-queue.js?v=8&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
import {
...
import { resolvePersistedAttributeClassification } from '../../js/equipment-attribute-classifications.js?v=3&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
...
function normalizeAttribute(attribute, fallbackKey = '') {
  if (attribute && typeof attribute === 'object' && !Array.isArray(attribute)) {
    ...
    return { label, value: attribute.value ?? '' };
  }
...
function renderClassification(draft) {
  ...
  for (const [raritySlug, attributes] of Object.entries(draft.variants || {})) {
    for (const attribute of attributes || []) {
      ...
      } else if (!rule.recognized) {
        detailRows.push({
          type: 'bad',
          title: `${humanizeAttributeKey(attribute.label)} · ${raritySlug}`,
          text: 'Regra ainda sem destino matemático ou classificação. O cadastro será permitido e a pendência será enviada à Auditoria.'
        });
      } else if (/aproximada/i.test(rule.source || '')) {
        detailRows.push({
          type: 'info',
          title: `${humanizeAttributeKey(attribute.label)} · ${raritySlug}`,
          text: `Correspondência automática: ${targetLabel(rule.target)} · ${rule.operation === 'percent' ? 'percentual sequencial' : 'soma direta'}.`
        });
      }
    }
  }
  ...
  box.innerHTML = `
    <h3>Classificação automática dos atributos</h3>
    <p>Esta classificação usa o mesmo catálogo do motor e as decisões persistentes da Auditoria.</p>
    <div class="json-classification-summary">
      <span class="json-classification-pill ok">${result.recognized} reconhecido(s)</span>
      <span class="json-classification-pill info">${result.approximate} por correspondência automática</span>

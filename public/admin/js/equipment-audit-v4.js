...
import {
  ...
  targetLabel
} from '../../js/equipment-audit-rules.js?v=3&pgv=82901afbc8df';
import {
  refreshEquipmentAttributeClassifications
} from '../../js/equipment-attribute-classifications.js?v=2&sb=20260823-security-supabase-pin-1&pgv=82901afbc8df';
...
function attributeAction(group) {
  if (group.problems.includes('unknown-rule')) {
    return 'Abra o equipamento e confirme o significado. Se for apenas outra escrita de uma regra existente, padronize o nome. Se for uma mecânica real cuja base não é pública, marque como “Aguardando dado oficial”. Se houver uma base real, crie a regra própria; se não existir cálculo numérico, mantenha como efeito especial/informativo.';
  }
  if (group.problems.includes('invalid-value')) {
...
function renderPending() {
  ...
  if (!attributeProblems.length && !bonusPending.length) {

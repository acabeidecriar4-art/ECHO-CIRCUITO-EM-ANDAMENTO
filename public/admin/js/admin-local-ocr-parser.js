...
function buildLines(readings = []) {
  ...
  for (const reading of readings) {
    String(reading && reading.text || '')
      ...
      .map(text => text.trim())
      .filter(Boolean)
      .forEach(text => rows.push({
        text,
        clean: normalize(text),
        source: reading && reading.name || 'imagem',
  ...
  return rows.filter(row => {
    const key = (row.region || 'full') + '|' + row.clean;
...
function equipmentAttribute(line, ocrConfidence) {
  ...
  const label = labelMatch
    ? labelMatch.label
    : String(line).replace(parsed.raw, ' ').replace(/^[\s:;,+\-–—]+|[\s:;,+\-–—]+$/g, '').trim();
...
export function parseEquipmentOcr(readings = [], context = {}, identity = {}) {
  ...
  if (!name) {
    for (const row of lines.slice(0, 20)) {
      if (row.text.length < 4 || row.text.length > 60) continue;
      if (/\d/.test(row.text)) continue;
      if (/conjunto|efeitos|categoria|equipamento|bonus/i.test(row.clean)) continue;
      if (rarityForLine(row.text, context.rarities || [])) continue;
      name = row.text;
      break;
    }
  }
  ...
  for (const row of lines) {
    ...
    if (currentBonus) {
      currentBonus.stats.effects.push({
    ...
    if (currentRarity) {
      if (!variants[currentRarity].some(item => item.raw === attribute.raw)) {
        variants[currentRarity].push(attribute);

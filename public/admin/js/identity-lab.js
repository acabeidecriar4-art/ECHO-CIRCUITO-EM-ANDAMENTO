...
async function selectUser(userId){
  ...
  const [identity,reputation,badges]=await Promise.all([
    ...
    supabase.from('echo_community_reputation').select('user_id,reputation_points,community_tier,verified_count,corroborated_count,first_discoveries,acceptance_rate,accepted_count,decided_count,policy_version,updated_at').eq('user_id',userId).maybeSingle(),

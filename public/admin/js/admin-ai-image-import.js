...
import { parseLocalOcr } from './admin-local-ocr-parser.js?v=20260828-hero-ocr-5&pgv=82901afbc8df';
import { recognizePaddleAttempts } from './admin-neural-ocr.js?v=20260828-paddle-browser-v3&pgv=82901afbc8df';
...
function mergeReadingTexts(readings) {
  const seen = new Set();
  const lines = [];
...
async function buildOcrAttempts(file, entityType) {
...
async function recognizeTesseractAttempts(
  ...
  const confidenceValues = partial
    .map(item => Number(item.confidence) || 0)
    .filter(Boolean);

  return [{
    name: file.name,
    text: mergeReadingTexts(partial),
    confidence: confidenceValues.length
      ? confidenceValues.reduce((sum, value) => sum + value, 0) /
        confidenceValues.length
      : 0,
    method: partial.map(item => item.method).join(' + '),
    region: 'full',
    engine: 'tesseract-5.1.1',
    neural: false
  }];
...
function summaryHtml(entityType, normalized) {
  ...
  return `
    ...
    ${recognizedHtml}
    ${warnings.map(warning => `<div class="admin-ai-warning">⚠ ${escapeHtml(warning)}</div>`).join('')}
